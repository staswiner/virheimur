...
void main() {
	...
	gl_Position = explode(gl_in[0].gl_Position, normal);//+ vec4(gs_in[0].Depth * 10);
	TransferData(0);								   //
	EmitVertex();									   //
	gl_Position = explode(gl_in[1].gl_Position, normal);//+ vec4(gs_in[1].Depth * 10);
	TexCoords = gs_in[1].texCoords;					   //
	Normal = gs_in[1].Normal;						   //
	FragPos = gs_in[1].FragPos;						   //
	EmitVertex();									   //
	gl_Position = explode(gl_in[2].gl_Position, normal);//+ vec4(gs_in[2].Depth * 10);
